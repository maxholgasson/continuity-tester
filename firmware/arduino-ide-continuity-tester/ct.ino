/* Main program - Arduino IDE version */
/* Target: ATmega88PA @ 1 MHz internal oscillator */
...
static uint8_t adc_measure(uint8_t channel)
{
	...
	for (uint8_t i = 0; i < nr_samples; i++) {
		ADMUX = channel & 15;
		ADCSRA = 1<<ADEN | 1<<ADSC | 1<<ADPS1 | 1<<ADPS0;  /* 250 kHz / 8 = ~125 kHz */
		while (ADCSRA & (1<<ADSC))
			memory_barrier();
		acc += ADC;
	}
